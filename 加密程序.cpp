...
const char c2[94]=":{3?dIgAV*kjcrH>+\\8)wx~&PSU_5v]<`T@#(JB$zNR\"e\'YMt/;O6m[lqQh-92uKpW7FGo|%Znb1yE.f}L=sD^4CX0ia!";
...
inline string compress2(mpz_class x){
	if(x==0)return {c2[0]};string s;while(x)s+=c2[mpz_class(x%93).get_si()],x/=93;reverse(s.begin(),s.end());return s;
}
inline mpz_class decompress(const string &s){mpz_class num=0;for(char a:s)num=num*93+int(find(c2,c2+94,a)-c2);return num;}
...
LRESULT CALLBACK PrimeTableWndProc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam){
	switch(msg){
		case WM_CREATE:{
			...
			if(hFont)SendMessage(hList,WM_SETFONT,(WPARAM)hFont,TRUE);PostMessage(hWnd,WM_COMMAND,1004,0);break;
		}
...
inline void showPrimeTable(){
	...
	print<<L"正在启动素数表查看器...\n";isrun=true;if(CreateThread(NULL,0,PrimeTableThread,NULL,0,0))
	print<<L"素数表查看器已启动\n您可以选择合适的素数用于密钥生成\n您还可以使用右键菜单复制选中的素数\n";
...
void work(const int &id){
	...
	for(int i=0;i<input[id].size();i+=M){
		mpz_class x;mpz_urandomb(y.get_mpz_t(),state,1988-8*M);
		for(int j=i;j<i+M;j++)x=(x<<8)|(j<input[id].size()?(unsigned char)(input[id][j]):0);
		x|=(mpz_class(min(M,int(input[id].size())-i))<<8*M)|(mpz_class(id)<<8*M+8)|(y<<8*M+12);
...
int main(int argc,char *argv[]){
	...
	if(p)s="("+compress2(p*q)+","+compress2(d)+")";clear(p);clear(q);clear(d);for(int i=0;i<N;++i)clear(prime[i]);
	...
	if(!s.empty()){
		if(savepath=="-null")a=0;else if(savepath=="-no")a=1;else if(!savepath.empty())print<<L"自动私钥中..."<<flush,a=save();
